...
(() => {
  ...
  // public/js/utils/show-alert.ts
  var hideAlert, showAlert;
  var init_show_alert = __esm({
    "public/js/utils/show-alert.ts"() {
      "use strict";
      hideAlert = () => {
        var _a;
  ...
  // public/js/utils/validate-email.ts
  var ValidateEmail;
  var init_validate_email = __esm({
    "public/js/utils/validate-email.ts"() {
      "use strict";
      ValidateEmail = (mail) => {
  ...
  var require_main = __commonJS({
    "public/js/main.ts"(exports) {
      ...
      btn.addEventListener("click", (e) => __async(exports, null, function* () {
        e.preventDefault();
        if (subject.value.trim().length < 1) {
          showAlert("error" /* ERROR */, "Please provide a subject", 4);
          return;
        }
        if (recipients.value.length < 1) {
          showAlert("error" /* ERROR */, "Please provide one or more recipients", 4);
          return;
        }
        const getRecipients = recipients.value.length === 1 ? [recipients.value] : recipients.value.split(",").filter((x) => x != "");
        ...
        if (message.value.trim().length < 1) {
          showAlert("error" /* ERROR */, "Please enter your message", 4);
          return;
        }
        btn.innerHTML = "Sending...";
        try {
          const response = yield axios_default.post("/email", {
            subject: subject.value.trim(),
            to: emailRecipients,
            message: message.value.trim()
          });
          showAlert("success" /* SUCCESS */, response.data, 4);
          subject.value = "";
          message.value = "";
          recipients.value = "";
        } catch (error) {
          showAlert("error" /* ERROR */, error, 4);
        } finally {
          btn.innerHTML = "Send";
        }
      }));
